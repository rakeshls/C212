...
def browseFiles():
    ...
    try:
        ...
        if(message[:4]=='send'):
            ...
            final_message = message +' '+str(filesize)
            SERVER.send(final_message.encode())
            textarea.insert(END,'File successfuly send')
...
def sendMessage():
    ...
    if(msgtosend == 'y'or msgtosend =='Y'):
        textarea.insert(END,'\n'+'Please wait file is downloarding')
        textarea.see('end')
        HOSTNAME = "127.0.0.1"
        USERNAME = "lftpd"
        PASSWORD = "lftpd"
        home= str(Path.home())
        download_path = home+'/downloards'
        ftp_server = ftplib.FTP(HOSTNAME,USERNAME,PASSWORD)
        ftp_server.encoding = 'utf-8'
        ftp_server.cwd('shared_files')
        ...
        file.close()
        ftp_server.quit()
        print('File successfuly downloarded')
        textarea.insert(END,'\n'+'File downloarded')
        textarea.see('end')
